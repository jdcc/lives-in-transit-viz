...
GoogleUrl = (function() {
  function GoogleUrl(sourceIdentifier) {
    ...
    if (this.sourceIdentifier.match(/http(s)*:/)) {
      ...
      try {
        this.grid = "od" + (6 + parseInt(this.url.match(/gid=(.*?)&/)[1]));
      } catch (error) {
        this.grid = this.url.match(/(od\d+)\//)[1];
      }
    } else {
      this.key = this.sourceIdentifier;
    }
    this.jsonCellsUrl = "http://spreadsheets.google.com/feeds/cells/" + this.key + "/" + this.grid + "/public/basic?alt=json-in-script";
    this.jsonListUrl = "http://spreadsheets.google.com/feeds/list/" + this.key + "/od" + this.grid + "/public/basic?alt=json-in-script";
